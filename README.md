...
The **bootstrap-and-clone-and-go arc is feature-complete**, **dependency-hardened** (zero known advisories in the production tree), and runs as a **single-user local deploy** — a built-artifact run path (`node dist/index.js`) with a documented single-command setup (`scripts/setup.sh`). The full v1 runtime is in place: methodology bundle loading and hot-reload, the items, sessions, and capture tracker, the reconcile engine, library, directives, discipline-drift and code-drift detection, GitHub polling, the methodology gate runtime, companion review, session-start scaffolding, the RAG/intelligence layer, backup, and a cost meter. 610 backend + 214 frontend tests pass; CI (`typecheck` / `test` / `lint` / `build`) is the required status check on `main`. *(Status as of the 2026-06-07 audit-remediation cohort, which cleared the supply-chain exposure and deploy gaps the 2026-06-06 end-to-end audit found; see `docs/_meta/throughline/audits/2026-06-06-end-to-end-summary.md`. "Feature-complete" is not the top `production-ready` taxonomy state — see below.)*
