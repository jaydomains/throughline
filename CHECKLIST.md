...
- [ ] **Slice 2 — Frontend: SettingsView `DisciplineScanBlock`.** Sibling of `BootstrapBlock` inside `ProjectSection`; prominent when state is `pre-scan`, demoted re-scan affordance when `complete`; consumes `DisciplineDriftRescanResult`.
