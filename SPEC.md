...
The CLI surface for first-touch bind is `throughline init`. It requires the backend to be running, probes `/health` before any write, and writes only via existing HTTP endpoints — never the datastore directly (T-D52). *(F1-04: prose aligned to the implementation's `/health` route, per the 2026-05-27 T-D52 amendment.)*
...
Re-import classifies every row into one of three states: new (insert), existing with no user edits since last import (update in place), and existing with user edits since last import (queued in the review queue for per-row resolution: `keep_mine`, `take_theirs`, or `merge_fields` *(deferred)*). Rows whose `bootstrap_id` was present in a prior import but is absent from the current one are flagged `bootstrap_stale=true`; never auto-deleted. The review queue surfaces stale rows with `keep` / `archive` *(deferred)* / `delete` actions.

*The `merge_fields` (per-field conflict merge) and `archive` (stale-row archive) sub-actions are **deferred** (audit M-6, 2026-06-07): v1 ships the working alternatives — `keep_mine` / `take_theirs` for conflicts and `keep` / `delete` for stale rows. The deferred sub-actions are a larger feature build (per-field merge UI; an archive capability for items/sessions/library entries, which v1 has only for projects) and are scoped to their own future ROADMAP cohort with the design decisions and anchors that build requires — not this audit-remediation chain.*
...
| T-D52 | `throughline init` CLI requires the backend running; probes `/health` (amended 2026-05-27 to match the implementation route) and prints `Start the backend first: pnpm --filter @throughline/backend start` on failure; CLI writes only via existing HTTP endpoints, never the datastore directly | 7.26, 10 |
