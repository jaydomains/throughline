...
This section documents file paths and ownership, not field schemas. The structured shape of `bootstrap-output.json` itself is documented by T-D53 (per-source row format, bundle-aware validation, exclusions); the producer-side pipeline that writes and consumes these files is documented in SPEC §7.28 and C-D21. The `bootstrap_id` derivation rules per source type and the universal `@bootstrap-id:` override syntax for hand-authored rows are pinned in T-D54.
...
- T-D54 — `bootstrap_id` derivation rules and universal `@bootstrap-id:` override syntax
