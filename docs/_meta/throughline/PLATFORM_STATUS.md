...
**As of 2026-05-31.** **Phase E — Full Audit-Fix Close** chain is **in flight** (22-slice floor; one PR per slice). Plans on `main`: base `plans/2026-05-30-phase-e-full-audit-close.md` (E1–E18, E17a) + augmentation `plans/2026-05-31-phase-e-augmentation-feature-builds.md` (E19/E20/E21 + the E1 anchor-amendment mechanism). **Durable per-slice record (PR #, merge SHA, fix-rounds, halt/flake flags): `handovers/phase-e-execution-log.md` — read it for slice detail.** **Merged: E1–E14** (#88–#101; E14 at `9647dbc`). All three planned Phase E anchors are minted: **T-D60** (E1), **C-D26** (E5), **C-D25** (E6); E10 added the spec-author-ruled §7.10 clause (no anchor). **This slice — E15** — frontend races & error surfacing: generation-guards stop a superseded `ItemDetailPanel` refresh (S8-01) and a stale in-flight library search (S8-02) overwriting current data; the copy toast clears its timer on unmount (S8-03); `Board` inline-create surfaces a failed `createItem` instead of swallowing it (S8-04, DriftInbox half already closed in Phase C); and `PrBadges` + `GET /github/prs` now disclose the github-poller's health on the wire (`ProjectPrsResult.poll_healthy/poll_error`, threading E5's C-D26) so a fetch-fail / failing-poller reads distinctly from a genuine "none tracked" (SF6-09, T-D60). No new anchor. **Pre-existing test flakes flagged for Phase-F** (out-of-scope, halt-8; pass on retry/isolation, not chain regressions): `rag.test.ts` (real embedder under parallel load) + `directives.test.tsx` (frontend). **Prior cohort:** audit-fix A–D + Phases 19–22 remain `production-ready`. **Next:** E16 (audit-3 spec-contradiction bugs — F6-01, F1-03, F4-04, SF3-04) off updated `main` after E15 merges; **then the chain HALTS at E17** (spec-author product-decision gate).
...
**Status:** E1–E14 merged (#88–#101); E15 (frontend races & error surfacing) is the current slice. Anchors so far: **T-D60** (E1), **C-D26** (E5), **C-D25** (E6) — all three planned Phase E anchors minted. A T-D10 amendment is planned in E20 (augmentation); any anchor beyond T-D60/C-D25/C-D26/T-D10-amendment trips halt-class 5. E15–E16 + E17a mint no anchors.
**Next concrete action:** Merge E15 on green gate; branch E16 (audit-3 spec-contradiction bugs) off updated `main`. **E16 lands, then the chain HALTS at E17** — the designed spec-author product-decision gate (halt-class 9).
...
| PR | Title | Handover |
|---|---|---|
| _this PR_ | Phase E / E15 — Frontend races & error surfacing (S8-01/02/03/04, SF6-09): refresh/search gen-guards, toast-timer cleanup, Board create-error surfacing, PrBadges + `/github/prs` poll-health disclosure | `handovers/phase-e-execution-log.md` (§E15) |
