...
**Status:** E1–E26 merged (#88–#108; latest `f224147`). Merged appended slices: **E22** (F7-04 audit-log filters), **E23** (SF2-07/08 methodology-parse visibility), **E25** (F6-02 tier-1 path-stem + F1-02 reinit audit row), **E26** (F1-04/F8-01/I8 doc-fixes). **E17a — DEFERRED-MAJOR** (2026-06-01 ruling, Option 1): the live registry showed every remaining advisory now needs a *major*, so all dep remediation is deferred into the **future fastify-v5 major-migration phase** (LBD-5); the deferral is recorded only (no code). **Carried-forward advisories:** fastify `4.29.1`→5, fast-uri `2.4.0` (rides fastify 5), vite `5.4.21`→6 (path-traversal fix `>=6.4.2`), esbuild `0.21.5` (rides vite 6, fix `>=0.25.0`), protobufjs `6.11.6`→7 (transitive via optional `@xenova/transformers`; forced override runtime-risky for embeddings → rides the major phase). Anchors so far: **T-D60** (E1), **C-D26** (E5), **C-D25** (E6); a T-D10 amendment is planned in E20. Any anchor beyond T-D60/C-D25/C-D26/T-D10-amendment trips halt-class 5.
**Next concrete action:** Land the E17a deferral-record PR, then continue the chain: **E24** (SF5/SF6 frontend-swallow rides — *parked pending spec-author pointers; the six Lows lack documented locations*) → **E18** (closure-verification appendix; F4-04 verified-closed) → **E19 → E20 → E21** (augmentation feature builds; E20 carries the T-D10 amendment).
