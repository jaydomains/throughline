...
- **Fix-rounds:** 1 (Gitar "approved with suggestions" — a real minor edge: if the first `send('welcome')` write fails, `cleanup` ran but the dead connection was still `hub._add`'d, lingering in the client set until `closeAll`. Folded an `if (cleaned) return;` guard before registration. The SSE-route socket path is integration-tested per the established convention — the guard is inspection-verified, consistent with the events route's existing test approach.)
- **CI flake observed (recorded per the "flake is a finding" discipline):** the `gate` workflow reported a failure on the *superseded* commit `cec923a` (run `26723697312`). Not reproduced: the full gate (`pnpm -r typecheck && test && lint && build`) passes locally end-to-end, and the current HEAD's first `gate` run passed on identical code (the only diff to `cec923a` was a doc-only PR-number line). No CI-log fetch tool was available to root-cause; characterized as a transient infra/flake on a superseded commit. Watch: if it recurs on a clean commit, escalate (stabilise or quarantine per `AUTO_CONTINUE_WORKFLOW.md`). The merge gates on the green current HEAD.
- **Halt-class fires:** none (one flake, non-reproducing, on a superseded commit — not the circuit-breaker's "same finding 3 rounds").
- **Surfaces to spec author:** none.
