...
### E25 — Cluster-B residual bugs (F6-02, F1-02) · spec-author-ruled bugs
- **Branch:** `claude/phase-e-e25-clusterb-bugs`
- **PR:** #pending (draft → ready on green)
- **Merge SHA:** pending
- **Closes:** F6-02, F1-02 (audit-3; spec-author ruled BUG in cluster B). **Class:** spec-contradiction / spec-alignment bug.
- **F6-02 (tier-1 match: path-stem vs message-substring):** `github/tiers.ts` runTier1 documented (header comment) that an annotation maps to a verifier rule when its **title equals the rule id** OR its **flagged path contains the rule_path stem** — but the code matched `title===rule_id` **or `raw_details.includes(rule_id)` or `message.includes(rule_id)`** (substring over-match: a rule id mentioned anywhere in the annotation prose falsely fired) and **never implemented the path-stem match**. Fix: a new `ruleStem()` (basename without extension) + the documented match (`title===rule_id || path.includes(stem)`); the two over-matching substring checks are dropped. **SPEC §7.14 Tier-1 amended** to state the match contract explicitly (spec-author ruling: code aligned to documented intent; substring-on-message not used). Spec + code land together.
- **F1-02 (re-init audit rows):** `projects/service.ts` `update()` ran the per-field audit loop even for a re-init (`reinit_throughline: true`), emitting one row per changed field instead of the declared single `project_reinit` row (which didn't exist in code). Fix: a re-init now records **one `project_reinit` row** with the per-field detail as structured payload (`trigger_context.changed_fields`); a normal update keeps its per-field rows (each is its own operational signal). Per the spec-author ruling: "per-field rows are noise; the single row carries the operational signal; field-level detail lands as structured payload on the single row."
- **Files:** `github/tiers.ts` + `SPEC.md §7.14` (F6-02), `projects/service.ts` (F1-02), `test/github.test.ts` + `test/projects.test.ts` (extended).
- **Anchor:** none. **Deps:** none (F6-02 sits on the E16 tier-1 work already on main; no overlap with the F6-01 done-filter).
...
- **Fix-rounds:** 1 — Gitar flagged the empty-query short-circuit returning `via: 'semantic'` even when the substrate is unwired (overclaims a healthy retrieval). Reordered so the substrate is resolved *before* the empty-query check: unwired ⇒ `semantic-unavailable` regardless of query; wired-but-empty ⇒ honest `semantic` empty. +1 test assertion. (Chose the reorder over Gitar's re-resolve-the-provider variant — same honesty, no duplicated provider call.)
...
### E20 — Session-start full inputs (F4-01) · augmentation feature-build · **T-D10 amendment**
- **Branch:** `claude/phase-e-e20-session-start-inputs`
- **PR:** #pending (draft → ready on green)
- **Merge SHA:** pending
- **Closes:** F4-01 (Mod, BUILD). **Class:** feature-build. **Scope split:** delivers the **core** (E20a) — the LLM-assist spec-revision authoring surface is split to **E20b** (see below).
- **Spec basis:** SPEC §7 line 392 / C-D9 step 2 — session-start assembles 7 declared inputs; the engine omitted 2: **project-spec excerpts** + **execution-plan slice** (`engine.ts:196-246`).
- **Fix:**
  - **Execution-plan slice:** `assemble()` reads `bundle.templates.execution_plan` whole (single bundle-global string; not mode-keyed — "for the chosen mode" is satisfied by mode-driven template *selection*, A2 disposition). Absent ⇒ block omitted.
  - **Project-spec content:** reads the canonical `project_spec` library entry **directly** (`library.list({projectId, type:'project_spec'})[0]`, current state at read time, bounded to 8000 chars) — NOT via the semantic substrate (resolves A3). Absent ⇒ block omitted.
  - Both are added to `AssembledContext`, folded into the input fingerprint (a spec edit busts the re-render cache — covered by a test), and **prepended as labelled blocks** (`## Project spec`, `## Execution plan`) ahead of the include-prompt prefix + template, so they land regardless of template placeholders (mirrors the T-D12 include-block pattern).
- **`project_spec` type (T-D10 amendment — the planned E20 anchor touch; halt-class-5 pre-resolved by the 2026-05-31 A8 ruling):**
  - **Shared:** `project_spec` added to `LIBRARY_ENTRY_TYPES` (fifth canonical type).
  - **Migration `0013`:** SQLite can't `ALTER` a `CHECK`, so `library_entries` is **rebuilt** with the widened `type` CHECK. The rebuild reproduces the **full current schema** (all 15 columns across 0001/0004/0006/0012) and **all three indexes**, **preserves rowid** (the FTS5 external-content index is rowid-keyed), and **recreates the three FTS sync triggers** verbatim. Safe DROP — no incoming FK to `library_entries`. (First table-rebuild migration in the repo; established the pattern.)
  - **Create-time uniqueness:** `project_spec` is one-per-project — a second create throws `ProjectSpecExistsError` (422). Canonical identity is **structural** (type tag + uniqueness), no flag field. T-D47 intact (library-entry stratum, not a core item type).
  - **Frontend:** `LibraryView` type-label map gains `project_spec` (typecheck-forced; T-D59 wire-shape).
- **Anchor: T-D10 amendment** (DECISIONS.md + SPEC.md §14 T-D10/T-D43 rows + §7.8 prose + §12 summary; T-D43 kept consistent — whiteboards still deferred, distinct from `project_spec`). **No new anchor number** (amendment, not a mint). Halt-class-5 fired-and-resolved by the prior ruling.
- **Files:** `shared/src/library.ts`, `db/migrations/0013_phase_e_project_spec.sql` (new), `library/service.ts` (uniqueness + error), `methodology/session-start/engine.ts` (2 inputs), `frontend/src/views/LibraryView.tsx`, `DECISIONS.md`, `SPEC.md`, `test/library.test.ts` + `test/session-start.test.ts`.
- **Tests:** session-start (+3) — both blocks present with content (exec-plan via an injected `### Template: execution_plan` fixture); both omitted when absent; a spec edit changes the prompt (fingerprint not stale). library (+1) — one-per-project (second rejected; other projects unaffected; the project_spec is FTS-searchable after the rebuild). Backend **600 pass** (61 files); frontend 197; full typecheck/lint/build green.
- **Fix-rounds:** TBD.
- **Halt-class fires:** **5 (anchor touch)** — the T-D10 amendment; **pre-resolved** by the 2026-05-31 spec-author A8 ruling (amendment, not a new mint). No estimate breach on the core (the migration landed cleanly).
...
- **Tests:** intelligenceView (+2) — a rejected `getPeriodicReview` ⇒ `load-error-intelligence` carrying the message; two concurrent failures ⇒ both surface (accumulated). settingsView (+2) — a rejected `getCostSummary` ⇒ `load-error-settings`; two concurrent failures ⇒ both surface. Frontend suite **201 pass** (31 files); backend 603; typecheck/lint/build green.
- **Fix-rounds:** 1 — Gitar flagged that a single shared `loadError`/`panelError`/`bootstrapError` slot showed only the *last* of concurrent failures (partially re-introducing the swallow). Changed all three to **accumulated `Error[]`** rendered as multiple `<LoadError>`s, so every concurrent failure surfaces; +2 regression tests (both-fail ⇒ two banners).
- **Halt-class fires:** none. (No invented findings; six-ID accountability recorded; three genuine residual swallows fixed.)

### E20b — Project-spec LLM-assist revision surface (split from E20) · feature-build
- **Branch:** `claude/phase-e-e20b-spec-llm-assist`
- **PR:** #pending (draft → ready on green)
- **Merge SHA:** pending
- **Closes:** the E20 "size-up" deliverable the spec author split off (LLM-assisted `project_spec` authoring). **Class:** feature-build (AI-assist UX). **Ruling (2026-06-01):** build it; AI-assist only — suggest/draft/accept-reject, user-mediated, **never auto-edit**; reuse an existing pattern if one exists, else build minimally and flag it as the new pattern.
- **No existing reuse target:** there was no "draft → review → accept" AI-assist pattern in the codebase (chat *proposes* into the dump-zone review modal; companion *judges*; reconcile *applies diffs* — none is an inline draft-and-accept on an entry). E20b **establishes** that pattern; the module comment flags it for future LLM-assist features to follow.
- **Backend (`library/spec-assist.ts`, new):** `createSpecAssistService.draft(projectId, instruction)` reads the current `project_spec` entry, calls Anthropic for a revised markdown draft, and **returns it without persisting** (draft-only). Capability-gated: AI absent ⇒ `{draft:null, used_ai:false, status:'unavailable'}`; call throws/empty ⇒ `status:'failed'` — **never a fabricated draft (T-D60)**. Records cost (`project_spec_assist`, T-D29) + audits the call by **prompt fingerprint, never the spec body** (`field:'project_spec_draft'`, `applied:false`, T-D24). `registerSpecAssistRoutes` → `POST /api/projects/:id/library/project-spec/draft`. Wired in `server.ts` (model `project_spec_assist`).
- **Frontend (`LibraryView`):** a `SpecAssistPanel` (exported) shown in the entry editor for `project_spec` entries — an instruction input + "Draft revision" → preview → **Accept** (loads the draft into the editor and persists via the existing `onPatch` library-update path — the AI never writes; the user's click is the mediation) / **Reject** (discards). Unavailable/failed states disclosed inline, no fake draft. `api.draftProjectSpec` client added.
- **Files:** `library/spec-assist.ts` (new), `server.ts` (wire), `frontend/src/api.ts` (client), `frontend/src/views/LibraryView.tsx` (SpecAssistPanel + editor hook), `test/spec-assist.test.ts` (new), `test/specAssist.test.tsx` (new), `test/fixtures/mockApi.ts`.
- **Anchor:** none (reuses AnthropicClient + T-D60/T-D24/T-D29). **Deps:** rides E20a (`project_spec` type, merged). Independent of E24.
- **Tests:** backend (3) — drafts via AI **without persisting** (no project_spec written) + audits `applied:false` + records cost; AI-unavailable ⇒ no draft (T-D60); AI-throws ⇒ `failed`, no draft. frontend (3) — draft previews then applies **only on Accept** (`onAccept` not called before); Reject discards; unavailable ⇒ honest inline error, no preview, no apply. Backend **606 pass** (62 files); frontend **204 pass** (32 files); typecheck/lint/build green.
- **Fix-rounds:** TBD.
- **Halt-class fires:** none.
