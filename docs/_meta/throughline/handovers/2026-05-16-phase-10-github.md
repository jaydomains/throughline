...
Tests: backend 175 (was 160 baseline this session), frontend 79 (was 77), typecheck clean across all 3 packages. `pnpm lint` is the documented no-op (carry-forward 6a-9).
...
| `pnpm lint` no-op | repo | Carry-forward 6a-9. | Relied on `pnpm typecheck` + full suites (backend 175, frontend 79). |
...
- [ ] **Auto-reconcile undo durability.** 24h undo is in-memory (correct for the single-process backend) + audit provenance; a process restart drops live undo tokens. Landing site: revisit if multi-process or restart-survival is required.
