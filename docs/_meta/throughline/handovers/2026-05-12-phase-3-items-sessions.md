...
| Bundle-derived item types and statuses | built | `packages/backend/src/items/policy.ts`; `GET /api/projects/:id/policy` | Freeform → type=`task`, statuses=`['open','done']`, single board `Tasks`. The Phase 7 rich bundle plugs into the same helper. |
...
| Bundle-defined boards | built | `bundleItemPolicy` returns `boards`; `Board.tsx` renders per board; freeform yields `[{id:'tasks',label:'Tasks',type:'task'}]` | the Phase 7 rich-bundle will extend the policy to return todo + decision boards. |
...
- Phase 7 (rich bundle): `bundleItemPolicy` extends to declare multi-board layouts (todo + decision) and per-type statuses. No changes needed in any of the existing callers — they consume the policy as data.
