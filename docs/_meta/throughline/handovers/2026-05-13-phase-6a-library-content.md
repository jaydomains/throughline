...
- **Awaited result, not state, when chaining async actions.** React state updates are async; code subsequent to a `setX` call in the same handler closure sees the old value of `x`. Pattern: `const result = await api.x(); setStateFromResult(result); useResultDirectly(result)`. First applied in `PromptFillModal.onPrimaryClick` — the "Render & copy" button awaits `onRender()` which returns the rendered string, then passes that string straight to `navigator.clipboard.writeText`. Reading `rendered` from state in the same closure would see `null` and skip the copy. Expected to recur in **Phase 6c** (the md-ingest summarisation result feeds both the library entry write and the UI summary preview in one handler) and **Phase 10** (the PR-reconcile result feeds both the audit-log row and the user-facing toast in one handler).
- **Composite-action button labels commit to single-click execution of both halves.** A button labelled "Render & copy" must complete both actions in one click; "X & Y" labels are part of the API contract, not a flavour hint. Future review passes should verify button labels match button behaviour. The regression test `"Render & copy" completes both actions in a single click (composite-action contract)` in `packages/frontend/test/library.test.tsx` locks this for the prompt-fill modal; analogous tests should accompany any future composite-action buttons.
...
| "Render & copy" button only rendered, never copied | `packages/frontend/src/components/PromptFillModal.tsx` | The initial primary-button handler called `onRender()` when `rendered === null` but never chained the copy — the user had to click a second time. The button label "Render & copy" promises one-click execution of both. Reading `rendered` from state in the same closure would also have failed because `setRendered` is async. Caught by gitar review. | Refactored: `onRender` now returns the rendered string; `onPrimaryClick` awaits it and passes the result straight to `navigator.clipboard.writeText`. The button label is now an honest contract. Regression covered by `"Render & copy" completes both actions in a single click (composite-action contract)` in `packages/frontend/test/library.test.tsx`. Two conventions written into Last Decision Minted: awaited-result-not-state for async chains, and composite-action labels commit to single-click execution. |
