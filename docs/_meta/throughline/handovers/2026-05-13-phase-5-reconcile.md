...
- **Validate-then-transact for apply-style endpoints.** Any endpoint operating on a list of entity IDs supplied by the client validates ownership-against-the-route-context *before* any mutation, then performs mutations inside a transaction so partial failure rolls back. Two sub-rules: (1) walk the entity references in the request, bulk-fetch them in one query (avoid N+1), and reject if any belong to a different project / parent than the route context implies; (2) wrap mutations in `db.transaction(...)` so all-or-nothing semantics hold even when a row mid-loop throws (e.g. a since-deleted item). `reconcile/service.ts` apply is the reference. Same pattern is load-bearing for Phase 9 (discipline-drift apply), Phase 10 (GitHub auto-reconcile and auto-apply), Phase 14 (RAG-proposed mutations that route through review). Caught by gitar review on Phase 5; logged under Drift Flags below with regression tests.
...
- **Done-equivalent status convention.** `bundleDoneStatus()` uses `policy.statuses[last]` as a heuristic. Holds for freeform + the rich bundle; if a future bundle declares a non-`done` terminal status, this needs a T-D anchor to formalise the contract. Surface it via the bundle policy's section rather than a string-last hack.
...
| Cross-project item mutation via tampered diff | `packages/backend/src/reconcile/service.ts` apply | A run's `diff_json` round-trips through the client at apply time. The original implementation passed `r.item_id` directly to `items.update` and `drift.createCodeSignal` without validating that the items belonged to the run's project — a malicious or buggy client could pass IDs pointing at items in another project, letting one project's reconcile mutate another's items and misattribute drift signals. Caught by gitar review. | Pre-flight ownership check: walk every non-rejected, non-`new` row, bulk-fetch the referenced items, throw `CrossProjectMutationError` (HTTP 422) if any item's `project_id` doesn't match the run's. Validation runs *before* any mutation. Regression covered by `test/reconcile.test.ts` "rejects an apply whose diff references an item from another project" (asserts both the throw and the unchanged foreign item + still-pending run). |
| Apply not transactional — partial mutation on mid-loop failure | `packages/backend/src/reconcile/service.ts` apply | The original apply loop performed mutations one row at a time without a surrounding transaction. If `items.update` threw on row N (e.g. a row referencing a since-deleted item), rows 0..N-1 had already mutated state and the run was left `pending` — Throughline's view of project state subtly wrong (some items updated, some not, drift signals fired for half a diff). Caught by gitar review. | Wrapped the apply loop + run status update + top-level `reconcile_apply` audit row in `db.transaction(...)`. better-sqlite3 transactions are synchronous; every mutation in the loop (items.update, items.create, drift.createCodeSignal, appendAudit) is synchronous already. Regression covered by `test/reconcile.test.ts` "apply is transactional: a mid-loop failure rolls back earlier mutations" (deletes an item between pre-flight and the per-row dispatch to trip items.update mid-loop; asserts the earlier item's title is unchanged and the run is still pending). |
