...
| F4-04 | Minor | C-D12 — create/update already validate status **membership** against the type's lifecycle (`items/service.ts:528-533/605-609`); only transition **adjacency** is unenforced. **C-D12 wording ambiguity** (one-line says "enforce transitions", body says "status-only" — audit-3 §load-bearing flagged it). | open | ~~bug → E16~~ → **decision → E17** (re-classified 2026-06-01; see E16 note) |
...
- **(c) C-D2 reframe: T-D60 SUPERSEDES the offline-fallback sanction.** C-D2 is **narrowed to "capability-absent honest-distinct mode" only**; the offline *silent* fallback is **removed**. This is a real **C-D2 amendment** — landed cleanly in E1 with the spec-amendment markdown (amend the C-D2 body in `CODE_SPEC.md`; T-D60 in `DECISIONS.md`/`SPEC.md §14` records the superseding principle).
  > **Mechanism re-ruled by OQ6 (2026-05-31).** The *substance* of (c) stands; the *landing site* changes — the C-D2 narrowing is landed as a **T-D60 supersession note** (not a C-D2 body edit), with a one-line C-D2 status pointer. E1 executors apply `plans/2026-05-31-phase-e-augmentation-feature-builds.md` §"Anchor amendment" in place of the "amend the C-D2 body" instruction here.
...
- **C-D2 amendment (lands in this slice):** ~~edit the C-D2 body in `CODE_SPEC.md` to narrow it to "capability-absent honest-distinct mode" only, with a `> Amendment (2026-05-30, Phase E / E1): superseded-in-part by T-D60` block~~; mint **T-D60** in `DECISIONS.md` + `SPEC.md §14` (refresh `CODE_SPEC.md:9` count 59→60). This is the clean spec-amendment, not a silent code change.
  > **Mechanism re-ruled by OQ6 (2026-05-31) — supersedes the struck text above.** Do **not** edit the C-D2 body. Land the narrowing as a **T-D60 supersession note** (DECISIONS.md + SPEC §14) plus a one-line non-normative C-D2 status pointer. Authoritative mechanism: `plans/2026-05-31-phase-e-augmentation-feature-builds.md` §"Anchor amendment."
...
- **Files:** `github/tiers.ts` + `github/tier4.ts` + `github/poller.ts` (F6-01, SF3-04), `projects/service.ts` (F1-03). **Tests:** done-item filter on tiers 1-3; create rejects mismatch; sub-fetch failure doesn't clear. **Deps:** rebase w/ E14 (`projects/service.ts`). **Note:** **F6-02 is explicitly NOT here** — SPEC/CODE_SPEC are silent (only a code comment documents path-stem intent), so it's a value judgment → E17.
- **F4-04 re-classified → E17 (spec-author ruling 2026-06-01).** The plan's "clean bug" verdict was the upstream error: audit-3's §load-bearing list *already* flags **C-D12 transitions** as a wording ambiguity ("one-line summary says enforce, body says status-only") that "points at spec edits, not code fixes." The code already matches C-D12's **body** — status **membership** is validated at create + update (`items/service.ts:528-533/605-609`); only transition **adjacency** is unenforced, and reconcile / auto-reconcile-on-merge / dump-zone-apply all set `open→done` directly (`reconcile/engine.ts:190`), so enforcing adjacency would silently force a C-D12 amendment + a three-caller cascade into a bug-fix slice. F4-04 therefore rides to **E17** as a C-D12-wording decision item (next to F3-01). **Hardener lesson (for the cohort close):** *an audit finding tagged "bug" may be a wording-ambiguity in disguise — verify the spec body agrees with its one-line summary before treating it as a code defect.* E16 ships at **3-of-4** findings closed.
...
- **Surfaces:**
  ...
  - **Spec-silent / value-judgment:** F6-02 (path-stem vs message-substring), F1-02 (single `project_reinit` row vs per-field), F3-01 (§7.14-vs-T-D57 scan-gate-layer ambiguity), **F4-04** (C-D12 transitions: one-line "enforce" vs body "status-only" — re-routed from E16 on 2026-06-01; membership is already enforced, the open question is whether adjacency must be, given reconcile/dump-zone jump straight to done).
