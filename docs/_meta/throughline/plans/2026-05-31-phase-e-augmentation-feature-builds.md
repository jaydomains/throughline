...
- **Plumb both through:** add `executionPlanSlice` + `specExcerpts` to `AssembledContext` (`engine.ts:80-93`); gather in `assemble()` (`:193-247`); add `execution_plan_block` + `spec_excerpts_block` to the render call (`:359-370`) and the bundle templates' variable set. **Critical (silent-failure guard):** hash both new inputs into `inputFingerprint` (the `assemble()` fp loop `:214-234`) — omitting them would re-serve a stale cached prompt when only spec/plan changed (a fingerprint-staleness silent failure → halt-class 6 if a test reds it). The paired test asserts the fingerprint moves.
...
- **Anchor:** none (extends C-D9). **Deps:** **E1 merged** (honest embedder, shared with E19) and the **pre-existing `text-index.search`** substrate on `main` — **not** E19. E19 and E20 are **siblings**; E20 queries the substrate directly with a project-doc (`imported_doc`)-scoped filter rather than reusing E19's broader `entity_type === 'library'` routing. No E19→E20 sequencing edge. **Est. LOC:** 120–190.
...
- **E19 ∥ E20 (siblings, no edge):** both consume the **pre-existing** `text-index.search` substrate (post-E1 honest embedder). E19 wraps it into the library surface filtered to `entity_type === 'library'`; E20 queries it directly with a project-doc (`imported_doc`)-scoped filter for spec excerpts. Neither depends on the other — both depend only on **E1**. No E19→E20 sequencing edge (the earlier "soft dep" was a substrate/slice conflation, corrected per auditor A3).
