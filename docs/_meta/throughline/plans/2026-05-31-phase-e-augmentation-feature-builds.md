...
- **Anchor impact (updated after the spec-author identifier ruling).** E19 and E21 mint no anchor (reuse C-D2/T-D60/C-D9). The E1 anchor-amendment refines only the *mechanism* of the already-planned T-D60 mint + C-D2 narrowing. **E20, however, now requires a new anchor** — the spec-author-ruled dedicated `project_spec` type is a type-taxonomy change (T-D10 amendment, or a new methodology-agnostic-carve-out T-D, depending on the placement call). That is **beyond the planned T-D60/C-D25/C-D26 → it trips halt-class 5 (unplanned anchor)**, surfaced here and on PR thread A8 for the spec author to rule (placement + which anchor). The chain's halt-class-5 would also catch it at execution; surfacing it now is the discipline-correct pre-emption.
...
- **Fix:** wire the two missing inputs into the assembly pipeline:
  ...
  2. **Project-spec content:** **read the current state of the designated canonical `project_spec` typed item directly** and include its content (bounded length). Per the **A7 + identifier rulings** (2026-05-31): the canonical item is a **dedicated new `project_spec` type**, marked canonical **structurally** (the type itself + a **one-per-project uniqueness constraint enforced at create-time validation** — no flag field, no config pointer). This is a **direct typed-item read, not a substrate/semantic query** — it does **not** route through E19 or `text-index.search` (resolves **A3 turn-2**). **Read semantics (ruled):** session-start reads the item's **current state at read time**; no caching survives a spec edit (see the fingerprint guard below). E19 may index the item in its corpus but does not gate this read.
...
- **Residual surfaced (placement + anchor — do NOT silently bake).** Two consequences of the new type must go back to the spec author:
  - **(a) Placement vs the methodology-agnostic core.** The ruling says "new **item type**" + "**item-edit** flows" (→ the *items* pool). But core item types are **bundle-owned** (SPEC §7 line 129 + T-D47: "core has no hardcoded… all come from bundles"; freeform declares the single `task` type) — a core-reserved `project_spec` item type present in *every* project regardless of bundle is a **carve-out from that principle**. The clean alternative that matches A7's "library *item*" wording is a **library-entry type** (the `LibraryEntryType` set is **core-owned** — `library.ts:6` `['note','prompt','snippet','imported_doc']`, T-D10 — so adding `project_spec` is a clean extension of a core-owned set). **Recommend: library-entry type** (avoids the §129 conflict; matches A7). **Spec author to confirm placement** — it changes which subsystem E20 touches.
  - **(b) Anchor / halt-class-5 (resolves auditor A8).** Either placement is a **type-taxonomy change requiring an anchor**: a library-entry type **amends T-D10** ("four content types"; T-D43 deferred the 5th=whiteboards); an items-pool type needs a **new methodology-agnostic-core-carve-out T-D**. Either way E20 now needs an anchor **beyond the planned T-D60/C-D25/C-D26 → this trips halt-class 5 (unplanned anchor)**, surfaced to the spec author per the base plan's blessed halt set. The specific anchor (T-D10 amendment vs new T-D/C-D) is the spec author's to mint, gated on the placement call in (a).
- **Files (E20 grows — per the identifier ruling "size E20 up"):** the **`project_spec` type definition** (in the core-owned type set — `packages/shared/src/library.ts` `LIBRARY_ENTRY_TYPES` if library-entry placement, else the items type surface — pending the placement call); a **schema migration** if the new type/constraint needs one; the **create-time one-per-project uniqueness validation** (mirrors the E16 create-time validators, e.g. F1-03/F4-04); the **LLM-assisted revision surface** (user-mediated suggest/draft/accept-reject, reusing the existing item-edit AI-assist pattern); the designated-item read + `packages/backend/src/methodology/session-start/engine.ts` plumb; `packages/shared/src/` wire/type additions; bundle template variable docs.
...
- **Anchor:** **NOT "none" — E20 now requires an anchor (halt-class-5 trip; resolves auditor A8).** The new canonical `project_spec` type is a type-taxonomy change: **T-D10 amendment** (library-entry placement) **or** a **new methodology-agnostic-carve-out T-D** (items-pool placement) — beyond the planned T-D60/C-D25/C-D26. Surfaced to the spec author; the specific anchor is gated on the placement call above. (Session-start *wiring* itself still just extends C-D9; the anchor is for the new type.) **Deps:** **independent of both E19 and E1** — E20's inputs are a **direct typed-item read** (project spec) + the **bundle `execution_plan` template**; neither touches the embedder or `text-index.search`. (Refines the spec author's "both depend only on E1" parenthetical: E19 rides E1's honest embedder; E20 does not. Confirmed by the spec author's independence ruling.) No E19→E20 edge. **Est. LOC: ~220–340** (raised per "size E20 up": + new type def + migration + create-time uniqueness validation + LLM-assist revision surface) — now the largest of the three builds; **watch halt-class 4 (estimate breach)** at execution.
