...
- **Fix:** wire the two missing inputs into the assembly pipeline:
  ...
  2. **Project-spec content:** **read the designated canonical project-spec library item directly** (identifier TBD, pending the spec-author sub-ruling below) and include its content (bounded length). Per the **A7 ruling** this is a **direct library-item read, not a substrate/semantic query** — so it does **not** route through E19 or `text-index.search` (this also resolves **A3 turn-2**: no residual E19 coupling, no over-broad `entity_type === 'library'` source). E19 may index the item for its own search corpus but does not gate this read.
- **Plumb both through:** add `executionPlanSlice` + `projectSpec` (the designated-item content, bounded) to `AssembledContext` (`engine.ts:80-93`); gather in `assemble()` (`:193-247`); add `execution_plan_block` + `project_spec_block` to the render call (`:359-370`) and the bundle templates' variable set. **Critical (silent-failure guard):** hash both new inputs into `inputFingerprint` (the `assemble()` fp loop `:214-234`) — omitting them would re-serve a stale cached prompt when only spec/plan changed (a fingerprint-staleness silent failure → halt-class 6 if a test reds it). The paired test asserts the fingerprint moves.
- **Spec-author ruling (A7, 2026-05-31) + open sub-decision.** The *source* is **ruled**: a **designated canonical project-spec library item, read directly** (not the substrate). **Open sub-decision — do NOT guess, surfaced to the spec author via the overseer:** the item's **identifier** — *which item type it is and how it's marked canonical.* Spec-author hint: probably a project-level config field or a specific item-type designation; with no `Project.spec` field today this is likely a **small net-new** (bears on E20's LOC band, like the A2 net-new note). Until ruled, E20 is written against *"the designated canonical project-spec library item (identifier TBD, pending spec-author sub-ruling)."*
- **Files:** `packages/backend/src/methodology/session-start/engine.ts`; the designated-spec-item read (library service lookup by the TBD identifier); the **canonical-marking mechanism** (project-level config field *or* an item-type designation — the small net-new, pending the identifier sub-ruling); `packages/shared/src/` (if `AssembledContext`/`SessionStartPromptResult` is wire-exposed, and for any new config/marking field); bundle template variable docs.
- **Tests:** `test/methodology/session-start/full-inputs.test.ts` — assembled prompt contains the designated-spec-item block + execution-plan block; fingerprint changes when the spec item or execution-plan changes (cache not stale); freeform bundle → execution-plan block cleanly absent; project with no designated spec item → spec block cleanly absent (graceful).
- **Anchor:** none (extends C-D9). **Deps:** **independent of both E19 and E1** (post-A7). E20's inputs are a **direct library-item read** (project spec) + the **bundle `execution_plan` template** (A2) — neither touches the embedder or `text-index.search`, so E20's earlier E1/substrate dependency dissolves. (This *refines* the spec author's "both depend only on E1" parenthetical: E19 still depends on E1's honest embedder; E20, under the direct-read ruling, no longer does — surfaced for confirmation.) No E19→E20 edge. **Est. LOC:** 120–190, **plus** any small net-new to mark the canonical spec item (identifier sub-decision, TBD).
