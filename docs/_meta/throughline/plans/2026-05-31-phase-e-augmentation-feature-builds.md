...
- **Status: DRAFT — in planner↔auditor review.** Not yet approved. Draft→ready conversion and merge remain the **overseer session's** gate, not flipped here.
...
- **Base plan location verified.** `git ls-remote --heads origin` → `refs/heads/main = b215a05`; the base plan file is on that tree; my working HEAD equals it; my augmentation diff against `main` is exactly this one new file.
...
- **No new anchor minted.** All three builds reuse existing substrates/anchors (C-D2 text-embedding substrate; T-D60 honesty principle from E1; C-D9 session-start pipeline). The anchor amendment refines the *mechanism* by which E1 lands the already-planned **T-D60** mint and **C-D2** narrowing — it mints nothing beyond the base plan's three accepted anchors (T-D60/C-D25/C-D26). **No halt-class-5 (unplanned anchor) trip.**
...
- **Source-of-data verified:** *Execution-plan slice* — the bundle declares an `execution_plan` template (`packages/shared/src/bundle.ts:146`, parsed at `bundle-parser/templates.ts:40`), and C-D9 step 2 scopes it "**for the chosen mode**" — so the source is the bundle's mode-scoped `execution_plan` template (absent for freeform bundles → block omitted gracefully). *Project-spec excerpts* — there is **no** first-class `Project.spec` field (`packages/shared/src/project.ts:10-31`); the canonical project-doc content lives in the **text/RAG substrate** as ingested project-doc chunks + imported-doc library entries (CODE_SPEC line ~1005), so "excerpts" = top-relevance chunks from that substrate (i.e. the **E19** text-index search, mode/slice-scoped). This makes **E20 a soft consumer of E19** (see Sequencing). The one genuine residual ambiguity — *is "the project spec" canonically the ingested project doc, or a designated library entry?* — is surfaced as a slice sub-note, not silently baked (see E20 detail).

**E21 / F5-04 — dump-zone proposal has no primary-unit suggestion/route.**
- `packages/shared/src/capture.ts` — `ProposedItem` (`:18-37`) carries `target_session_id` (cross-session re-route, `:30`) but **no** primary-unit field; `ProposalPayload` (`:48-60`) carries `suggested_session_id` (`:57`) but **no** `suggested_primary_unit`. The file-header comment (`:11-14`) names cross-session re-route as the captured routing axis; primary-unit routing is absent.
...
- **F7-06 (full command palette):** SPEC §7.24 line 432 (jump to any project/session/item/library/directive/primary-unit/gate; run new-item/reconcile/export/switch-view/switch-project/settings). Code: `packages/frontend/src/components/CommandPalette.tsx` `PaletteItem.kind` is `'project' | 'view' | 'action'` (`:10-16`) with only project-open / view-jump / projects-list actions (`:40-86`) — partial. Audit-3 F7-06 (Major*).
...
- **F5-01 (tree grouping):** SPEC §7.11 line 268 — five grouping dims (by **tag, session, primary unit, status, blocker**). Code: `TreeView.tsx:10` `type GroupBy = 'status' | 'tag' | 'parent'` (the `<select>` at `:31-37` offers status/tag/parent) — ships ~3, missing the spec dims **session, primary unit, blocker**. Audit-3 F5-01 (Major): "ships 3 of 5 dims."
...
- **Base-plan E1 mechanism being revised:** base plan lines 250-251 instruct E1 to "**edit the C-D2 body in `CODE_SPEC.md`** to narrow it … with a `> Amendment` block." OQ6 supersedes this: land the narrowing as a **T-D60 supersession note**, **not** a C-D2 body edit (§"Anchor amendment" below).
...
- **Files:** `packages/backend/src/library/service.ts` (replace stub), `packages/backend/src/library/routes.ts` (unchanged or wiring only), `packages/backend/src/intelligence/text-index.ts` (expose library-filtered hits if needed), `packages/shared/src/library.ts` (`via` enum + embedder-capability field, on-contract), composition root (inject index).
- **Tests:** `test/library/semantic-search.test.ts` — seeded entries returned ranked by cosine; embedder-absent → disclosed degraded (not empty-as-healthy); embed-failure ≠ honest-empty (the SF3-02 split surfaces through to the library result).
...
- **Fix:** wire the two missing inputs into the assembly pipeline:
  1. **Execution-plan slice (for the chosen mode):** read the bundle's `execution_plan` template (`bundle.ts:146`); include it mode-scoped. Freeform / no-`execution_plan` bundle → block omitted (graceful, mirrors the existing minimum-spec path).
  2. **Project-spec excerpts:** pull top-relevance project-doc excerpts from the text substrate (the **E19** library/ingested-doc semantic search, slice-scoped) — bounded excerpt length, relevance-ranked.
- **Plumb both through:** add `executionPlanSlice` + `specExcerpts` to `AssembledContext` (`engine.ts:80-91`); gather in `assemble()` (`:193-247`); add `execution_plan_block` + `spec_excerpts_block` to the render call (`:355-364`) and the bundle templates' variable set. **Critical (silent-failure guard):** hash both new inputs into `inputFingerprint` (the `assemble()` fp loop `:230-244`) — omitting them would re-serve a stale cached prompt when only spec/plan changed (a fingerprint-staleness silent failure → halt-class 6 if a test reds it). The paired test asserts the fingerprint moves.
- **Surfaced sub-decision (not baked):** *the canonical "project spec" source.* Plan grounds on **ingested project-doc chunks via the text substrate** (CODE_SPEC line ~1005 + C-D9 step 2 "excerpts"). If the spec author intends a *designated* spec library entry instead, that is a one-line source swap — flagged here for confirmation, not silently chosen. Resolve before E20 codes, or default to the substrate-excerpt reading.
- **Files:** `packages/backend/src/methodology/session-start/engine.ts`; `packages/shared/src/` (if `AssembledContext`/`SessionStartPromptResult` is wire-exposed); bundle template variable docs.
- **Tests:** `test/methodology/session-start/full-inputs.test.ts` — assembled prompt contains spec-excerpt + execution-plan blocks; fingerprint changes when the underlying spec/plan changes (cache not stale); freeform bundle → execution-plan block cleanly absent.
- **Anchor:** none (extends C-D9). **Deps:** **soft on E19** (spec-excerpt substrate); if E19 not yet merged, gate spec-excerpts behind the substrate's availability (degrade gracefully, T-D60-consistent). Sequence **after E19**. **Est. LOC:** 120–190.
...
|---|---|---|---|
| **F5-01** | Major | Tree grouping by the two missing spec dims (**session, primary unit, blocker**) | SPEC §7.11:268; `TreeView.tsx:10` |
...
**This section overrides the base plan's E1 instruction at lines 250-251** ("edit the C-D2 body … with a `> Amendment` block"). The narrowing of C-D2 lands as a **T-D60 supersession note**, **not** a direct C-D2 body edit. Mechanism, as it executes in **E1** (still the slice that mints T-D60):
...
3. **`CODE_SPEC.md` — C-D2 body is NOT rewritten.** Its Decision/Rationale/Implications prose stays as-is. The narrowing's substance lives in T-D60. *(Whether to add a single non-normative cross-reference line to C-D2 — e.g. a status pointer "narrowed by T-D60" — is a minor traceability choice; this plan recommends keeping C-D2's body untouched per the OQ6 directive and letting T-D60 carry the supersession. If the spec author prefers a bare pointer, that is a one-line non-substantive addition, surfaced not baked.)*
...
- **Added LOC (estimate):** E19 (120–180) + E20 (120–190) + E21 (120–190) ≈ **360–560 LOC** across 3 code/test slices, atop the base ~2,050–2,970. Calendar: +~3–4 working days atop the base ~3–4 weeks.
...
- **E19 → E20 (soft):** E20's project-spec excerpts read the text substrate E19 surfaces. Sequence E20 after E19; if E19 slips, E20 degrades spec-excerpts gracefully (T-D60-consistent) rather than blocking.
...
*This is the planner DRAFT. Status flips to `final — approved by planner` only after the auditor loop converges and the audit-ID set-diff gate holds across revisions.*
