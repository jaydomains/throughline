...
> **Amended 2026-06-07 (A2 / audit finding M-1, supply-chain remediation).** The Transformers.js package was renamed from the deprecated `@xenova/transformers@2` to its maintained successor `@huggingface/transformers@3`. Motivation: the old stack pinned the protobufjs Critical RCE transitively (`onnxruntime-web@1.14 → onnx-proto@4 → protobufjs ^6.8.8`, which an override alone could not lift); the successor's newer onnxruntime resolves `protobufjs@>=7.5.8`, clearing the Critical + four High + four Moderate protobufjs advisories. **Dependency-declaration correction:** it is — and remains — an **`optionalDependency`** (a lazy dynamic import with the T-D60 capability-absent fallback when the package or its first-launch model download is absent), **not** a hard direct dependency. The model (`Xenova/all-MiniLM-L6-v2`, 384-dim) and the lazy-import + disclosed-fallback contract are unchanged. The `@xenova/transformers` naming in the baseline Decision/Implications below is superseded by `@huggingface/transformers`.

### Decision
The text RAG substrate (T-D25) uses local embeddings generated in-process by `@huggingface/transformers` (Transformers.js; renamed from `@xenova/transformers` — see the 2026-06-07 amendment above) running a small sentence-embedding model (e.g., `all-MiniLM-L6-v2` ONNX). Embeddings stored in SQLite in a `text_embeddings` table; cosine similarity computed in-memory or via `sqlite-vss` if performance demands it.
