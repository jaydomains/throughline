...
- **Graph view**: implemented as a dependency-free, deterministic SVG renderer over a pure layout module (`views/graph/layout.ts` → `views/GraphView.tsx`) rather than `cytoscape.js` — the CODE_SPEC's "or similar" latitude, chosen because the runtime is offline/ephemeral (no registry guarantee) and a deterministic SVG layout is reproducible and jsdom-testable. Nodes are items; edges are parent-child (`parent_id`) + structured blockers (`blockers[]`, T-D8). "Show chains" highlights blocker dependency paths and structural root blockers. **Cross-session-mention edges and the communication-model layer (§7.11) are deferred** pending spec-author clarification: the `Item` model has no mention/reference field, and the bundle parser extracts only `communication_model.edge_types` — not the per-unit routing/producer contract those edges would be drawn from (see DECISIONS.md working notes WN-1b-a / WN-1b-b). The §11 DoD bullet ("nine view modes functional and switchable") is satisfied by the functional item graph; the two deferred sub-features are §7.11 detail, not separate DoD lines. Visual layer adopts the design-handoff "Direction A · dark" tokens **scoped to `.graph-view`** (`views/graph/graph.css`, `--gv-*` namespace); the full design-system adoption (theme.css swap, sidebar, settings keys, SSE hot-reload) is a separate redesign pass (WN-1b-c).
